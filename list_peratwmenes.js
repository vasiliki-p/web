document.addEventListener('DOMContentLoaded', function() {
    ...
    // Φόρτωση δεδομένων
    async function loadTheses(search = '') {
        const response = await fetch(`get_ypo_exetasi_theses.php?search=${encodeURIComponent(search)}`);
    ...
    // Κουμπιά ενεργειών
    function renderActionButtons(thesis) {
        let buttons = '';
    ...
    async function cancelThesis(thesisId) {
        ...
        const response = await fetch('cancel_anathesis.php', {
            method: 'POST',
    ...
    async function submitGrade(thesisId, isFinal) {
        const grade = prompt(`Εισάγετε βαθμό (0-10)${isFinal ? ' (Τελικός)' : ''}:`);
        if (grade === null || isNaN(grade)) return;
        
        const response = await fetch('submit_grade.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                thesis_id: thesisId, 
